...
```{r}
nyc.subsamp <- nyctaxi.data[(nyctaxi.data$timestamp_dt > as.POSIXct("2014-10-01 00:00:00"))&
...
test.data <- nyc.subsamp[(nyc.subsamp$timestamp_dt >= as.POSIXct("2014-10-28 00:00:00"))&                          
                            (nyc.subsamp$timestamp_dt <= as.POSIXct("2014-10-30 00:00:00")),]
```
...
ToDo : Justify time windows chosen to model pattern
ToDo : Find appropriate trigger to re-train model. Perhaps increasing cumulative residual error in a window?
...
```{r}
percent.error <- abs(as.vector(test.data$value) - as.vector(arima.model.auto.fcast$mean))/as.vector(test.data$value)
plot(percent.error)
```

```{r}
hist(as.vector(test.data$value) - as.vector(arima.model.auto.fcast$mean))
qqnorm(as.vector(arima.model.auto.fcast$mean))
qqline(as.vector(arima.model.auto.fcast$mean))
ks.test(arima.model.auto.fcast$mean, "pnorm")
```
...
# get index of error terms above cut off
